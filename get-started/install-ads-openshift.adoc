...
Red Hat OpenShift Container Platform （ OCP ）に Astra データストアプレビューをインストールするには、ネットアップサポートサイトからインストールバンドルをダウンロードし、この手順で説明するインストール手順を完了します。
...
. にログインします https://mysupport.netapp.com/site/["ネットアップサポートサイト"] Astra データストアプレビューバンドル (`2021.12_Astrundsstore.tar') をダウンロードします
. （任意）バンドルのシグニチャを確認します。
+
[listing]
----
...
.手順
. NAT テーブルの出力チェーンの現在の「 KUBE-HOSTPORTS 」ルールの行番号をメモします。
+
...
----
. 新しいルールを NAT テーブルに追加します。この場合、「 KUBE-HOSTPORTS 」エントリのすぐ上に追加します。
+
[listing]
----
$ sudo iptables -t nat -I OUTPUT 4 -s 127.0.0.1 -j KUBE-MARK-MASQ -p tcp --dport 9920
----
. 新しく追加されたルールが 'KUBE-HOSTPORTS' ルールのすぐ上の NAT テーブルに追加されていることを確認します
+
[listing]
----
$ sudo iptables -t nat -L OUTPUT --line-numbers
...
NOTE: Astra Data Store プレビュー版クラスタ内のすべてのノードで、次の手順を実行します。


NOTE: Astra データストアプレビュークラスタノードが再起動した場合は、この手順を繰り返します。
